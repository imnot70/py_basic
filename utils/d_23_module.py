...
# 函数和变量的作用域是通过加下划线前缀来实现的
# 正常的变量和函数是可以被直接引用的，如 abc，x123
# 如__xxx__这样的特殊变量，可以被直接引用，但是有特殊用途
# 因此正常开发一般不使用这种变量名
# 类似_xxx和__xxx这样的变量是私有的，不应该被直接引用，如_abc，_xyz
# 但是python本身并没有办法限制访问private变量，因此只是一种规则
